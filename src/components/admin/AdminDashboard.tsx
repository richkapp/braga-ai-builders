...
import CommunityFeatureSettings from './CommunityFeatureSettings';
import { communityConfig } from '@/config/community';

type AdminMode = 'overview' | 'members' | 'invites' | 'events' | 'voting' | 'ideas' | 'bug-reports' | 'settings';
type Props = { mode?: AdminMode };

const modes: Array<{ key: Exclude<AdminMode, 'overview'>; label: string; description: string; superAdminOnly?: boolean }> = [
  ...
  { key: 'settings', label: 'Settings', description: 'Turn community features and participation options on or off.', superAdminOnly: true }
];
...
export default function AdminDashboard({ mode = 'overview' }: Props) {
  ...
  const visibleModes = modes.filter((item) => !item.superAdminOnly || isSuperAdmin);

  return (
    <div className="space-y-6">
      <div className="card p-6">
        <p className="text-xs uppercase tracking-[0.2em] text-braga-300">Organizer tools</p>
        <div className="mt-2 flex flex-wrap items-center gap-3"><h1 className="text-3xl font-black text-white">{communityConfig.name} admin</h1>{isSuperAdmin && <span className="rounded-full bg-limewash px-3 py-1 text-xs font-black uppercase tracking-[0.12em] text-ink-950">Super admin</span>}</div>
        <nav className="mt-5 flex flex-wrap gap-2" aria-label="Admin sections">
          ...
          {visibleModes.map((item) => <a key={item.key} className={mode === item.key ? 'btn-primary' : 'btn-secondary'} href={`/admin/${item.key}`}>{item.label}</a>)}
        </nav>
      </div>

      {mode === 'overview' && <div className="grid gap-5 md:grid-cols-2">{visibleModes.map((item) => <a key={item.key} href={`/admin/${item.key}`} className="card p-6 transition hover:border-limewash/60"><h2 className="text-xl font-bold text-white">{item.label}</h2><p className="mt-2 text-sm leading-6 text-braga-100">{item.description}</p><span className="mt-5 inline-flex font-semibold text-limewash">Open {item.label.toLowerCase()} →</span></a>)}</div>}
      {mode === 'members' && <MemberManager isSuperAdmin={isSuperAdmin} />}
      {mode === 'invites' && <InviteManager />}
      {mode === 'events' && <EventManager />}
      {mode === 'voting' && <VotingManager isSuperAdmin={isSuperAdmin} />}
      {mode === 'ideas' && <IdeaModerator />}
      {mode === 'bug-reports' && <BugReportManager />}
      {mode === 'settings' && (isSuperAdmin
        ? <CommunityFeatureSettings />
        : <div className="card p-6"><h2 className="text-xl font-bold text-white">Super-admin access required</h2><p className="mt-2 text-sm leading-6 text-braga-200">Only super admins can change community-wide feature settings.</p></div>)}
    </div>
  );
}
