    ...
    <div className="space-y-7">
      ...
      <form onSubmit={preview} className="card space-y-5 p-5 sm:p-6" aria-busy={busy}>
        ...
        {!previewing ? <>
          ...
          <div><label className="label" htmlFor="vote-closes">Closing date and time</label><input id="vote-closes" className="input mt-2" type="datetime-local" value={closesAt} onChange={(event) => setClosesAt(event.target.value)} required /><p className="mt-2 text-xs text-braga-300">Displayed publicly in Lisbon time. Admins may close earlier.</p></div>
        ...
        </> : normalizedPreview && <section className="rounded-2xl border border-limewash/35 bg-ink-950/45 p-5" aria-labelledby="vote-preview-title">
          <p className="text-xs font-black uppercase tracking-[0.16em] text-limewash">Public preview</p>
          <h3 id="vote-preview-title" className="mt-3 text-2xl font-black text-white">{normalizedPreview.title}</h3>
          <p className="mt-3 whitespace-pre-wrap text-sm leading-6 text-braga-100">{normalizedPreview.description}</p>
          <p className="mt-3 text-xs text-braga-300">Closes {formatDate(normalizedPreview.closesAt)} · Lisbon time</p>
          <ol className="mt-5 space-y-2">{normalizedPreview.options.map((option) => <li key={option} className="rounded-xl border border-braga-300/20 px-4 py-3 text-sm text-white">{option}</li>)}</ol>
          <div className="mt-5 flex flex-col gap-3 sm:flex-row"><button type="button" className="btn-secondary flex-1" onClick={() => setPreviewing(false)} disabled={busy}>Back to edit</button><button type="button" className="btn-primary flex-1" onClick={() => void persist(true)} disabled={busy}>{busy ? 'Publishing…' : editingStatus === 'published' ? 'Save published changes' : 'Publish vote'}</button></div>
        </section>}
      </form>
