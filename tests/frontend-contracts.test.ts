...
describe('launch frontend contracts', () => {
  ...
  test('email-link sign in and signup stay short, explicit, and passwordless', async () => {
    ...
    expect(tabs).toContain('<InviteEmailForm mode="signin" onSignUp={() => chooseTab(\'signup\', true)} />');
    expect(form).toContain('New here?');
    expect(form).toContain('Sign Up →');
    expect(form).not.toContain('Get an invite from a friend who’s already a member');
    ...
    expect(form).toContain("{ mode: 'invite'; code: string; onSignUp?: never } | { mode: 'signin'; code?: never; onSignUp: () => void }");
    ...
    expect(form).toContain('New here?');
    expect(form).toContain('Sign Up →');
    expect(form).not.toContain('data-whatsapp-join');
  ...
  test('profile photos use the native upload flow instead of editable external URLs', async () => {
    ...
    expect(uploader).toContain('JPEG, PNG, WebP, HEIC, or HEIF');
    expect(uploader).toContain('accept="image/jpeg,image/png,image/webp,image/heic,image/heif,image/heic-sequence,image/heif-sequence,.heic,.heif"');
    expect(avatar).toContain("import('heic-to/csp')");
    expect(avatar).toContain("type: 'bitmap'");
    expect(avatar).toContain('release: () => bitmap.close()');
  ...
  test('organizer routes cover the complete v1 operations', async () => {
    for (const path of [
      ...
      'src/components/admin/CommunityFeatureSettings.tsx',
    ...
    expect(admin).toContain("key: 'settings'");
