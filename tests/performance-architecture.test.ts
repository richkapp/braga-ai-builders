...
describe('performance architecture contracts', () => {
  test('public pages build statically while parameterized routes stay on demand in Paris', async () => {
    ...
    expect(JSON.parse(vercel).regions).toEqual(['cdg1']);
    for (const route of [
  ...
  test('the persistent shell resolves browser auth and Voting visibility once', async () => {
  ...
  test('signed-out visitors skip admin work and bug-report code loads only on demand', async () => {
    ...
    expect(events).toContain('if (!user || isAnonymousUser(user))');
    expect(events.indexOf('if (!user || isAnonymousUser(user))')).toBeLessThan(events.indexOf('isCurrentUserAdmin()'));
